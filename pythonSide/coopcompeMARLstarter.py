...
module_specs = {}
for policy_name in ["team_0_policy", "team_1_policy"]:
    module_specs[policy_name] = RLModuleSpec(
...
config = (
    ...
    .training(
        lr=1.5e-4,
        train_batch_size=2048, #this is not a batch size but env steps in some cases, so for 8 cars the batch size is
        gamma=0.99,
        use_gae=True,
        lambda_=0.95,
        clip_param=0.2,
...
if args.paramCount:
    for policy in ["team_0_policy", "team_1_policy"]:
        module = algo.get_module(policy)
...
for i in range(1000000):
    ...
    for policy_name, learner in learners.items():
